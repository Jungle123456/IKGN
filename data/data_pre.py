...
class DataFoursquare(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=48, min_gap=10, session_min=3, session_max=10,
        ...
        tmp_path = "D:\STKGRec-test-inverse-category1\data\\"
        self.TWITTER_PATH = tmp_path + 'new_dataset_TSMC2014_TKY.txt'
        self.SAVE_PATH = tmp_path
        self.save_name = '/tky_min_category'
    ...
    def load_trajectory_from_tweets(self):
        with open(self.TWITTER_PATH, 'rb') as fid:
            for line in fid:
                ...
                s=re.split('[\t?\r\n]', line)
                uid, pid, category_id, _, lat, lon, _, UTC_time, _, _ = re.split('[\t?\r?\n]', line)
                ...
                if uid not in self.data:  # [u1]={[loc1, time1],[loc2,time2]}
                    self.data[uid] = [[pid, tim, category_id]]
                else:
                    self.data[uid].append([pid, tim, category_id])
                ...
                if category_id not in self.category_name:
                    self.category_name.append(category_id)
                self.category_name = list(set(self.category_name))
                num1 = 0
                for i in self.category_name:
                    self.category[i] = num1
                    num1 += 1
        ...
        for u in self.user_filter3:  #
            ...
            if u not in self.uid_list:
                self.uid_list[u] = [len(self.uid_list),
                                    len(sessions)]  #
            for sid in sessions:  #
                poi = [p[0] + "分" + p[2] for p in sessions[sid]]  # loc
                for p in poi:
                    if p.split("分")[0] not in self.vid_list:
                        self.vid_list_lookup[len(self.vid_list)] = p.split("分")[0]  # loc->loc_id
                        self.vid_list[p.split("分")[0]] = [len(self.vid_list), 1,
                                                          p.split("分")[1]]  #
                    else:
                        self.vid_list[p.split("分")[0]][1] += 1
    ...
    def prepare_neural_data(self):
        ...
        for u in tqdm(self.uid_list):
            ...
            for sid in sessions:  #
                ...
                self.tim_rel.extend([k[0] for k in gap_list])  # k[0]: delta-t
                self.dis_rel.extend([k[1] for k in gap_list])  # k[1]: delta-d
            ...
            self.data_neural[self.uid_list[u][0]] = {'sessions': sessions_check, 'train': train_id, 'test': test_id,
                                                     'vaild': vaild_id,
    ...
    def construct_data(self, n_tim_rel, tim_dis_dict):
        ...
        rel_ptp = [int(tim_dis_dict[tuple(triple[1])] + n_tim_rel) for triple in
                   self.train_ptp]  #
    ...
    def add_category(self):
        for i, j in self.vid_list.items():
            if i != 'unk':
                self.poi2category[j[0]] = self.category[j[2]] + len(self.uid_list) + len(self.vid_list)

        kg_train_data = pd.DataFrame(self.kg['train_kg'], columns=list('hrt'))

        for i, j in self.poi2category.items():
            temp = []
            temp.append(i)
            temp.append(max(kg_train_data['r']) + 1)  #
            temp.append(j)
            self.poi2category_list.append(temp)
        self.kg['train_kg'] = np.concatenate([self.kg['train_kg'], self.poi2category_list])  #
    ...
    def save_variables(self):
        foursquare_dataset = {'data_neural': self.data_neural, 'vid_list': self.vid_list, 'uid_list': self.uid_list,'category_name':self.category_name,
...
if __name__ == '__main__':
    ...
    data_generator.load_trajectory_from_tweets()  ## 1. read trajectory data from file
    print('filter users')
    data_generator.filter_users_by_length()  ## 2.0 basically filter users based on visit length and other statistics
    print('build users/locations dictionary')
    data_generator.build_users_locations_dict()  # 3. build dictionary for users and location
    # data_generator.load_venues()
    data_generator.venues_lookup()  # 映射loc->loc_id  ### 4. remap lon and lat
    print('prepare data for neural network')
    data_generator.prepare_neural_data()  ### 5.0 prepare training data for neural network
    data_generator.prepare_kg_data()  ### 5.0 prepare training data for neural network
    print('save prepared data')
    data_generator.save_variables()  # 6. save variables
